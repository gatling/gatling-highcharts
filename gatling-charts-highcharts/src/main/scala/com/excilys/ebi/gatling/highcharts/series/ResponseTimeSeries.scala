...
import com.excilys.ebi.gatling.charts.computer.Computer.AVERAGE_TIME_NO_PLOT_MAGIC_VALUE_AS_INT
...
class ResponseTimeSeries(name: String, data: List[(DateTime, Int)], color: String) extends Series[DateTime, Int](name, data, List(color)) {
	...
	override def isPlotMandatory(plot: (DateTime, Int)) = plot._2 != AVERAGE_TIME_NO_PLOT_MAGIC_VALUE_AS_INT
	...
	def getElements: ArrayBuffer[String] = {
		...
		if (!sample.isEmpty)
			buffer ++= sample.map {
				entry => new StringBuilder().append("[").append(entry._1.getMillis).append(",").append((if (entry._2 == AVERAGE_TIME_NO_PLOT_MAGIC_VALUE_AS_INT) "null" else entry._2)).append("]").toString
